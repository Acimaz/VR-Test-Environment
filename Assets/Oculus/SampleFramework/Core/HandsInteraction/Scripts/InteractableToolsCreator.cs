...
namespace OculusSampleFramework
{
	...
	public class InteractableToolsCreator : MonoBehaviour
	{
		...
		List<GameObject> leftHandTools;
		List<GameObject> rightHandTools;
		...
		private void AttachToolToHandTransform(Transform tool, bool isRightHanded)
		{
			...
			if (isRightHanded)
			{
				...
				{
					rightHandTools = new List<GameObject>();
				}
				rightHandTools.Add(newTool.gameObject);
			}
			else
			{
				if (leftHandTools == null)
				{
					leftHandTools = new List<GameObject>();
				}
				leftHandTools.Add(newTool.gameObject);
			}
		}
		
		public void DeactivateLeftHandTool()
		{
			if (leftHandTools == null || rightHandTools == null)
			{
				Debug.Log("Lefthandtools or Righthandtools is null");
				return;
			}
			foreach (GameObject item in leftHandTools)
			{
				item.SetActive(false);
			}
			foreach (GameObject item in rightHandTools)
			{
				item.SetActive(true);
			}
		}
		public void DeactivateRightHandTool()
		{
			if (leftHandTools == null || rightHandTools == null)
			{
				Debug.Log("Lefthandtools or Righthandtools is null");
