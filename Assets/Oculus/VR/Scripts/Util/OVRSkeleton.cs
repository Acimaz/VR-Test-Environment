...
public class OVRSkeleton : MonoBehaviour
{
	...
	[SerializeField]
	protected bool _updateRootScale = false;
	[SerializeField]
	...
	public bool physicsCapsulesTrigger;
	...
	private void InitializeCapsules(OVRPlugin.Skeleton skeleton)
	{
		if (_enablePhysicsCapsules)
		{
			...
			for (int i = 0; i < skeleton.NumBoneCapsules; ++i)
			{
				...
				capsuleColliderGO.layer = gameObject.layer; //CUSTOM: Needed for physics layers to work correclty
				...
				capsuleCollider.isTrigger = physicsCapsulesTrigger;
	...
	public void SwitchCapsuleTriggerStates(bool? enable = null)
	{
		foreach (OVRBoneCapsule capsule in _capsules)
		{
			capsule.CapsuleCollider.isTrigger = !capsule.CapsuleCollider.isTrigger;
		}
		physicsCapsulesTrigger = !physicsCapsulesTrigger;
	}
	public void EnableCapsulesTrigger(bool enable)
	{
		foreach (OVRBoneCapsule capsule in _capsules)
		{
			capsule.CapsuleCollider.isTrigger = enable;
		}
		physicsCapsulesTrigger = enable;
	}
	public void RebuildColliders()
	{
		foreach (OVRBoneCapsule capsule in _capsules)
		{
			capsule.CapsuleCollider.enabled = false;
			capsule.CapsuleCollider.enabled = true;
		}
	}
